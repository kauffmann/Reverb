...
// Use like `Hadamard<double, 8>::inPlace(data)` - size must be a power of 2     This preserve that output energy == input energy
template<typename Sample, int size>
class Hadamard {
public: 
	static inline void recursiveUnscaled(Sample * data) {
		...
		// Combine the two halves using sum/difference       This essentially mix channels so that one ch gets signal from other channels
	...
	static inline void inPlace(Sample * data) {
		...
		Sample scalingFactor = std::sqrt(1.0/size);  // the energy preserve part.
